...
---@class lspconfig.LspConfig
local LspConfig = {}
LspConfig.__index = LspConfig

---@param name string
---@param def lspconfig.ConfigDef
---@return lspconfig.LspConfig
function LspConfig:new(name, def)
  validate {
    name = { name, 's' },
    default_config = { def.default_config, 't' },
    on_new_config = { def.on_new_config, 'f', true },
    on_attach = { def.on_attach, 'f', true },
    commands = { def.commands, 't', true },
  }

  local deprecate = def.default_config.deprecate --[[@as { to?: string, version: string }]]
  if deprecate then
    vim.deprecate(name, deprecate.to, deprecate.version, 'lspconfig', false)
  end

  if def.commands then
    for k, v in pairs(def.commands) do
      validate {
        ['command.name'] = { k, 's' },
        ['command.fn'] = { v[1], 'f' },
      }
  ...
  local default_config = tbl_deep_extend('keep', def.default_config, util.default_config)
  -- Force this part.
...
function LspConfig:setup(user_config)
  ...
  local config = tbl_deep_extend('keep', user_config, self._default_config)
